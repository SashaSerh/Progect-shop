...
async function run() {
  ...
  const context = await browser.newContext({ viewport: { width: 375, height: 812 }, userAgent: 'mobile' });
  const page = await context.newPage();

  const fileUrl = serverUrl + '/';
  await page.goto(fileUrl, { waitUntil: 'networkidle' });

  // Force system font for deterministic screenshots
  await page.addStyleTag({ content: '* { font-family: system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial !important; }' });
  ...
  // Stabilize rendering after animation
  ...
  // Wait for page slide animation and content load
  await page.waitForSelector('#service-ac-install .service-page__title', { timeout: 4000 });
  await page.waitForTimeout(500); // stabilize rendering after animation
  ...
  if (!createBaseline) {
    ...
    function compare(imgA, imgB, out) {
      ...
      const mismatch = pixelmatch(a.data, b.data, diff.data, width, height, { threshold: 0.14 });
    ...
    if (totalMismatch > 10) {
      console.error('Visual differences exceed threshold (10 pixels). See latest diffs in', OUT_DIR);
      process.exit(2);
    }
