...
export function renderProductCard(product, lang, translations) {
    ...
    try {
        ...
        if (isPicsum600) {
            ...
            // Local image path (e.g., picture/name.jpg). We assume responsive siblings exist:
            // name-320w.ext, name-480w.ext, name-768w.ext, name-1200w.ext (generated by scripts/images generator)
            const m = baseSrc.match(/^(.*)(\.[a-zA-Z0-9]+)$/);
            if (m) {
                const base = m[1]; const ext = m[2];
                const s320 = `${base}-320w${ext}`;
                const s480 = `${base}-480w${ext}`;
                const s768 = `${base}-768w${ext}`;
                const s1200 = `${base}-1200w${ext}`;
                srcsetAttr = `${s320} 320w, ${s480} 480w, ${s768} 768w, ${s1200} 1200w`;
            } else {
                srcsetAttr = `${baseSrc} 600w`;
            }
        }
